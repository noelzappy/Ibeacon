...
class App extends Component {
  ...
  beaconsUpdateTick(updatedBeacons) {
    ...
    currentBeacons = currentBeacons?.filter(t => t.unliveTicks < unliveTicksLimit);
  ...
  async startRanging() {
    if (Platform.OS == 'android') {
      if (await this.requestLocationPermission()) {
        try
        {
          ...
          this.beaconsServiceDidConnect = Beacons.BeaconsEventEmitter.addListener(
            ...
            () => {
              ...
              Beacons
                .startRangingBeaconsInRegion(this.regionId)
        ...
        Alert.alert('Location permission', "Application couldn't work w/o permission.")
      }
    }
    ...
    if (Platform.OS == "android") {
      BeaconBroadcast.checkTransmissionSupported()
        .then(() => {
          ...
          if (status) {
            BeaconBroadcast.startAdvertisingBeaconWithString(this.state.uuid, this.state.identifier, 0, 0)
    ...
    return (
      <View style={{flexDirection: 'row', marginHorizontal: 20, marginBottom: 10}}>
        <View style={{width: 50, alignItems: 'flex-end'}}>
          <Text style={{fontSize: 24, fontWeight: 'bold'}}>#{item.index + 1}</Text>
        </View>
        <View style={{flexDirection: 'column', paddingLeft: 20, flex: 1}}>
          <Text>UUID: {item.item.uuid}</Text>
          <View style={{flexDirection: 'row'}}>
            <Text style={{color: '#AAAAAA', textTransform: 'capitalize', flex: 1}}>{item.item.proximity}</Text>
            <Text style={{color: '#AAAAAA'}}>~{item.item.distance.toFixed(2)}m</Text>
          </View>
          <Text style={{color: '#CCCCCC'}}>RSSI: {item.item.rssi}, Minor: {item.item.minor}, Major: {item.item.major}</Text>
        </View>
      </View>
    );
  }
        ...
        <View style={styles.content}>
          <View>
            <View style={{flexDirection: 'row', padding: 10, alignItems: 'center'}}>
              <Switch
                ...
                thumbColor={this.state.isTransmitterEnable ? "#00BFFF" : "#f4f3f4"}
                ios_backgroundColor="#3e3e3e"
                onValueChange={this.onTransmitterStatusChanged}
                value={this.state.isTransmitterEnable} />
              <Text style={{paddingLeft: 10, fontSize: 16}}>Enable transmitter</Text>
            </View>
            { this.state.isTransmitterEnable &&
              <View>
                <Text style={{paddingHorizontal: 10, paddingBottom: 10, color: "#AAAAAA", alignSelf: 'center'}}>
          ...
          <View style={{flex:1}}>
            <View style={{flexDirection: 'row', padding: 10, alignItems: 'center'}}>
              ...
              <Text style={{paddingLeft: 10, fontSize: 16}}>Enable listener</Text>
            </View>
            { this.state.isListenerEnable &&
              <View style={{flex:1}}>
                <Text style={{margin: 10, color: "#AAAAAA", alignSelf: 'center'}}>
